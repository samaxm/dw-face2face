...
public class EasemobApiUtil {
	...
	private String getToken() throws GetEasemobTokenException{
		...
		try{
			Response response=JSON(getTokenURL, object.toJSONString(), new HttpHeader(Content_Type, application_json));
	...
	public String registerEasemobUser(String userName,String password) throws EasemobRegisterFailed, GetEasemobTokenException{
		...
		int fail=0;
		try {
			Response response=JSON(registerUser, json.toJSONString(), new HttpHeader("Authorization", "Bearer "+token));
